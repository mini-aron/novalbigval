...
- **재미용 기능** — 슬래시커맨드가 아니라 평문 채팅 문구로 반응한다. 예: 채팅에 `세이지 날씨 서울`이라고 치면 날씨를 알려준다. (`src/features/`)

설계 배경과 전체 명세는 프로젝트 킥오프 시 정리한 명세 문서를 참고.

> 상점/위시리스트/전적 조회처럼 실제로 중요한 기능은 전부 슬래시커맨드로, 날씨 같은 부가 기능만 평문 채팅 트리거로 간다. 평문 채팅 트리거를 쓰려면 Discord 개발자 포털의 봇 설정에서 **Message Content Intent**를 켜야 한다 (기본은 꺼져 있음).
...
src/
  ...
  features/            평문 채팅 트리거(재미용, 슬래시커맨드 아님)
    weatherTrigger.ts   "세이지 날씨 <지역>" → Open-Meteo로 날씨 조회
